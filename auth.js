...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    if (document.getElementById("mobile-btn-google"))
      document.getElementById("mobile-btn-google").classList.add("hidden");
    if (document.getElementById("mobile-btn-email"))
      document.getElementById("mobile-btn-email").classList.add("hidden");
    if (document.getElementById("mobile-btn-logout"))
      document.getElementById("mobile-btn-logout").classList.remove("hidden");

    // Desktop Auth menu toggles
    if (document.getElementById("auth-btn-google"))
      document.getElementById("auth-btn-google").classList.add("hidden");
    if (document.getElementById("auth-btn-email"))
      document.getElementById("auth-btn-email").classList.add("hidden");
    if (document.getElementById("auth-btn-logout"))
    ...
    if (document.getElementById("auth-user-email"))
    ...
    if (document.getElementById("mobile-btn-google"))
      document.getElementById("mobile-btn-google").classList.remove("hidden");
    if (document.getElementById("mobile-btn-email"))
      document.getElementById("mobile-btn-email").classList.remove("hidden");
    if (document.getElementById("mobile-btn-logout"))
      document.getElementById("mobile-btn-logout").classList.add("hidden");

    // Desktop Auth menu toggles
    if (document.getElementById("auth-btn-google"))
      document.getElementById("auth-btn-google").classList.remove("hidden");
    if (document.getElementById("auth-btn-email"))
      document.getElementById("auth-btn-email").classList.remove("hidden");
    if (document.getElementById("auth-btn-logout"))
      document.getElementById("auth-btn-logout").classList.add("hidden");

    // Booking Form Protection
    if (document.getElementById("booking-auth-overlay")) {
      document
        .getElementById("booking-auth-overlay")
...
async function saveUserProfile(user) {
  try {
    ...
    alert(
      "Error de base de datos: No tienes los permisos configurados en Firebase o no se ha creado la base de datos Firestore. Ve a la consola y pega las Reglas de Seguridad.\nDetalle: " +
        error.message,
    );
...
getRedirectResult(auth)
  ...
  .catch((error) => {
    console.error(
      "Error al regresar de Google Redirect:",
      error.code,
      error.message,
    );
    alert(
      `Error al volver de Google [${error.code}]: ${error.message}\n\nDomino detectado: ${window.location.hostname}`,
    );
  });
// ==========================================
...
async function loadMiPerfilYHistorial(user) {
  ...
  try {
    ...
    if (!listEl) return;
    listEl.innerHTML =
      '<div class="text-center text-gold-500 py-6"><i class="fa-solid fa-circle-notch fa-spin text-2xl mb-2"></i><p class="text-sm">Cargando...</p></div>';
  ...
  if (btnEdit && formEdit && viewProfile && btnCancel) {
    ...
    formEdit.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        document.getElementById("modal-profile-username").textContent =
          "@" + newUsername;
...
window.loginWithEmail = async function () {
  const email = prompt(
    "Ingresa tu correo electrónico para enviarte un enlace de acceso mágico (sin contraseña):",
  );
  if (!email) return;
  ...
  try {
    ...
    alert(
      `¡Enlace mágico enviado! Revisa la bandeja de entrada de ${email} y haz clic en el enlace para entrar.`,
    );
  } catch (error) {
...
if (isSignInWithEmailLink(auth, window.location.href)) {
  ...
  if (!email) {
    email = window.prompt(
      "Por favor, ingresa tu email nuevamente para confirmar:",
    );
  }
...
async function loadMisCitas(container) {
  container.innerHTML =
    '<div class="text-center text-gold-500 py-10"><i class="fa-solid fa-circle-notch fa-spin text-3xl mb-3"></i><p>Cargando tus citas...</p></div>';

  if (!window.currentUserId) {
    container.innerHTML =
      '<p class="text-center text-gray-400 py-10">Inicia sesión para ver tus citas.</p>';
    return;
  }
  ...
  try {
    ...
    if (querySnapshot.empty) {
      container.innerHTML =
        '<div class="text-center py-10"><p class="text-gray-400 mb-2">No tienes citas registradas aún.</p><button onclick="document.getElementById(\'mis-citas-modal\').classList.add(\'hidden\'); document.getElementById(\'reservas\').scrollIntoView();" class="text-gold-500 underline">¡Reserva una ahora!</button></div>';
      return;
    }
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if (cita.status === "pending")
        statusBadge =
          '<span class="px-2 py-1 bg-yellow-500/20 text-yellow-400 rounded-full text-xs">Pendiente</span>';
      else if (cita.status === "confirmed")
        statusBadge =
          '<span class="px-2 py-1 bg-green-500/20 text-green-400 rounded-full text-xs">Confirmada</span>';
      else if (cita.status === "completed")
        statusBadge =
          '<span class="px-2 py-1 bg-blue-500/20 text-blue-400 rounded-full text-xs">Completada</span>';
      else if (cita.status === "cancelled")
        statusBadge =
          '<span class="px-2 py-1 bg-red-500/20 text-red-400 rounded-full text-xs">Cancelada</span>';
      else
        statusBadge = `<span class="px-2 py-1 bg-gray-500/20 text-gray-400 rounded-full text-xs">${cita.status}</span>`;
    ...
    container.innerHTML =
      '<p class="text-center text-red-400 py-10">Hubo un error al cargar las citas. Verifica la configuración de la base de datos (Security Rules e Índices).</p>';
  }
}
